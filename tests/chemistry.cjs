...
assert.ok(document.querySelector('.home-cover-actions a[href="?page=journey"]'),'Homepage has a Learn RNA path');
assert.ok(document.querySelector('.home-analyze-menu'),'Homepage has an Analyze RNA branch menu');
assert.ok(document.querySelector('.home-analyze-branches a[href="?page=secondary"]'),'Analyze menu has a 2D Secondary path');
assert.ok(document.querySelector('.home-analyze-branches a[href="?page=tertiary"]'),'Analyze menu has a 3D Tertiary path');
assert.equal(document.querySelectorAll('.home-path-card').length,0,'Redundant homepage Learn/Analyze cards are removed');
