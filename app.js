...
function selectResidue(index) {
  ...
  document.querySelectorAll(".secondary-node").forEach((node, i) => node.classList.toggle("selected", i === state.selectedResidue));
  renderTertiary();
}
...
function buildTrnaCoordinates() {
  ...
  for (let i = 9; i <= 12; i++) p[i] = { x: 270 - (i - 9) * 30, y: 190 };
  for (let i = 13; i <= 20; i++) {
    const angle = (-30 - (i - 13) * (300 / 7)) * Math.PI / 180;
    p[i] = { x: 125 + Math.cos(angle) * 55, y: 220 + Math.sin(angle) * 55 };
  }
  for (let i = 21; i <= 24; i++) p[i] = { x: 180 + (i - 21) * 30, y: 250 };
  p[25] = { x: 300, y: 245 };
  for (let i = 26; i <= 30; i++) p[i] = { x: 320, y: 270 + (i - 26) * 21 };
  for (let i = 31; i <= 37; i++) {
    const angle = (200 + (i - 31) * (140 / 6)) * Math.PI / 180;
    p[i] = { x: 350 + Math.cos(angle) * 50, y: 405 + Math.sin(angle) * 50 };
  }
  for (let i = 38; i <= 42; i++) p[i] = { x: 380, y: 354 - (i - 38) * 21 };
  p[43] = { x: 403, y: 255 };
  for (let i = 44; i <= 47; i++) p[i] = { x: 425 + (i - 44) * 17, y: 240 + (i % 2) * 7 };
  for (let i = 48; i <= 52; i++) p[i] = { x: 490 + (i - 48) * 20, y: 190 };
  for (let i = 53; i <= 59; i++) {
    const angle = (170 - (i - 53) * (340 / 6)) * Math.PI / 180;
    p[i] = { x: 625 + Math.cos(angle) * 54, y: 220 + Math.sin(angle) * 54 };
  }
  for (let i = 60; i <= 64; i++) p[i] = { x: 570 - (i - 60) * 20, y: 250 };
  for (let i = 65; i <= 71; i++) p[i] = { x: 370, y: 168 - (i - 65) * 18 };
  for (let i = 72; i <= 75; i++) p[i] = { x: 399 + (i - 72) * 31, y: 47 - (i - 72) * 3 };
  return p;
}
