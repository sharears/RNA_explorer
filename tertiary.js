const TertiaryExplorer = (() => {
  ...
  function setupControls(){
    ...
    box.innerHTML=
      ...
      '<label class="te-check"><input id="teShowPairs" type="checkbox" checked> Show all 2D pair guides (not hydrogen bonds)</label>'+
      ...
      '<label>Surface transparency<input id="teSurfaceOpacity" type="range" min="0.05" max="0.9" step="0.05" value="0.35"></label>'+
      '<label class="te-check"><input id="teSplit" type="checkbox"> 2D + 3D linked view</label>'+
      '</details>'+
      '<details open><summary>Select</summary><p class="te-tool-note">Click a residue here or in 3D; selection is shared with the Secondary view when mapping is active.</p><div id="teSequencePanel" class="te-sequence-panel"></div>'+
      ...
      '<details><summary>Display · residue indices</summary><p>Default labels: 1, every 5 residues, and the final residue.</p><details class="te-index-dropdown"><summary>Choose indices</summary><div id="teIndexChoices"></div></details><div class="te-button-row"><button type="button" id="teIndexDefault">Default</button><button type="button" id="teIndexAll">All</button><button type="button" id="teIndexNone">None</button></div></details>'+
      ...
      '</details>'+
      '<details><summary>Advanced · clipping</summary><label class="te-check"><input id="teClipEnabled" type="checkbox"> Enable clipping slab</label><div class="te-inline-grid"><label>Near<input id="teClipNear" type="range" min="-100" max="0" step="1" value="-40"></label><label>Far<input id="teClipFar" type="range" min="0" max="100" step="1" value="40"></label></div><p class="te-tool-note">Clipping changes only what is visible; it does not delete atoms.</p></details>'+
      '<details><summary>Advanced · compare / align</summary><label>Alignment scope<select id="teAlignScope"><option value="full">Whole active RNA chain</option><option value="selection">Current residue selection</option></select></label><label>Comparison PDB / mmCIF<input id="teAlignFile" type="file" accept=".pdb,.ent,.cif,.mmcif"></label><label class="te-check"><input id="teCompareVisible" type="checkbox" checked> Show aligned comparison</label><button type="button" id="teClearAlignment">Clear comparison</button><p id="teAlignmentStatus">Upload a second PDB/mmCIF structure to superimpose it on the active RNA chain.</p></details>'+
      '<details><summary>Advanced · saved camera views</summary><button type="button" id="teSaveView">Save current view</button><div id="teSavedViews">No saved views.</div></details>'+

      '<details id="teFocusDetails"><summary>Advanced · focus on structural region</summary><div class="te-button-row te-region-buttons"><button type="button" data-te-region="Acceptor stem">Acceptor</button><button type="button" data-te-region="Anticodon arm">Anticodon</button><button type="button" data-te-region="elbow">D/T-loop elbow</button><button type="button" data-te-region="full">Full structure</button></div></details>'+
    ...
    $("teSelectRange").addEventListener("click",selectRange);$("teSelectCurrent").addEventListener("click",addCurrentToSelection);$("teSelectSubtract").addEventListener("click",subtractCurrentFromSelection);$("teSelectChain").addEventListener("click",selectActiveChain);$("teSelectInvert").addEventListener("click",invertSelection);$("teSelectClear").addEventListener("click",()=>{state.directSelectionIndices.clear();state.selectedPairs.clear();refreshCombinedSelection();render();});
  ...
  function setupToolbar(){
    ...
    const context=document.createElement("section");context.id="teContextPanel";context.className="te-context-panel";context.hidden=true;
    context.innerHTML='<div class="te-context-heading"><strong>Selection</strong><span id="teContextSummary">0 residues selected</span></div>'+
    ...
    $("teApplySelectionStyle").addEventListener("click",applySelectionStyle);
    $("teResetSelectionStyle").addEventListener("click",resetSelectionStyle);
    $("teContextClear").addEventListener("click",()=>{state.directSelectionIndices.clear();state.selectedPairs.clear();refreshCombinedSelection();render();});
    $("tePairHBondCutoff").addEventListener("input",e=>{if(e.target.checkValidity()){state.pairHBondCutoff=Number(e.target.value);render();}});
